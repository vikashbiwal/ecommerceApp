...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        NSLog(@"frame x and y position %f %f",frame.origin.x,frame.origin.y);
...
- (void)mg_updateBadgeViewSize {
    ...
    NSLog(@"badge width :: %f",badgeWidth);
    NSLog(@"badge Height :: %f",badgeHeight);
    
    [self setBounds:CGRectMake(0,0, badgeWidth-2, badgeHeight-2)];
    
    CGRect superviewFrame = self.superview.frame;
    
     [self setFrame:CGRectMake(superviewFrame.size.width/2,5, badgeWidth-7, badgeHeight-7)];
}
...
- (void)mg_updateBadgeViewPosition {
    ...
    switch (position) {
        case MGBadgePositionTopRight: {
            NSLog(@"superviewFrame :: %f",superviewFrame.size.width);
             NSLog(@"superviewFrame/2 :: %f",superviewFrame.size.width/2);
            self.center = CGPointMake(superviewFrame.size.width-15, 0);
            break;
        }
        ...
        case MGBadgePositionCenterRight: {
            self.center = CGPointMake(superviewFrame.size.width/2+10, superviewFrame.size.height/2-8);
            break;
        }
        default:
            break;
