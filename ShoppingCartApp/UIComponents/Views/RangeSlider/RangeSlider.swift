...
class RangeSliderThumbLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        ...
        //add laebl on thumb layer
        
        // Flip the coordinate system
        ctx.textMatrix = CGAffineTransform.identity // 2-1
        ctx.translateBy(x: 0, y: self.bounds.size.height); // 3-1
        ctx.scaleBy(x: 1.0, y: -1.0); // 4-1
        
        
        let attrs = [NSFontAttributeName : UIFont.systemFont(ofSize: 8.0)]
        
        let gString = NSMutableAttributedString(string:displayValue, attributes:attrs);
        
        
        let line = CTLineCreateWithAttributedString(gString)
        ctx.textPosition = CGPoint(x:5, y:bounds.size.height/2) //
...
class RangeSlider: UIControl {
    ...
    func updateLayerFrames() {
        ...
        lowerThumbLayer.displayValue = String(format: "%.0f",lowerValue)
        lowerThumbLayer.setNeedsDisplay()
        
        let upperThumbCenter = CGFloat(positionForValue(upperValue))
        upperThumbLayer.frame = CGRect(x: upperThumbCenter - thumbWidth/2.0, y: 0.0, width: thumbWidth, height: thumbWidth)
        upperThumbLayer.displayValue = String(format: "%.0f",upperValue)
        upperThumbLayer.setNeedsDisplay()
